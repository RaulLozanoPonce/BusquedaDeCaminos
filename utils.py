...
class babg(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda ciudad: ciudad.path_cost, reverse=True)

    def pop(self):
        return self.A.pop()

class rasg(Queue):
    """Gestiona una lista ordenada con ramificación  y  acotación  con  subestimación"""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda ciudad: ciudad.path_cost + self.problem.h(ciudad), reverse=True)
